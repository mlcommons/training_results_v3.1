...
_cont_mounts=("--volume=${DATADIR}:/data" "--volume=${LOGDIR}:/results" "--volume=${PWD}/bind.sh:/bm_utils/bind.sh"
	"--volume=${PWD}/azure.sh:/bm_utils/azure.sh" "--volume=${PWD}/run_and_time.sh:/bm_utils/run_and_time.sh")
...
mllogger.event(key=mllogger.constants.CACHE_CLEAR, value=True)"
        ...
        docker exec -it "${_config_env[@]}" "${_cont_name}" \
	       mpirun --allow-run-as-root --bind-to none --np ${DGXNGPU} /bm_utils/run_and_time.sh
