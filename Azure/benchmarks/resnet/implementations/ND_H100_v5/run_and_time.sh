...
if [[ "${KVSTORE}" == "horovod" ]]; then
  ...
  #BIND="bindpcie --cpu=azure.sh --mem=azure.sh ${IB_BIND} --"
  #BIND="./bind.sh --cpu=azure.sh --mem=azure.sh ${IB_BIND} --"
  BIND="/bm_utils/bind.sh --cpu=/bm_utils/azure.sh --mem=/bm_utils/azure.sh --ib=off --cluster=${cluster} --"
fi
