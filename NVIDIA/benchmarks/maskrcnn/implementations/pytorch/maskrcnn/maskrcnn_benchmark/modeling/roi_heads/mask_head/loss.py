...
class MaskRCNNLossComputation(object):
    ...
    def prepare_targets(self, proposals, targets, syncfree, weights):
        if weights is None:
        ...
        for proposals_per_image, targets_per_image, weights_per_image in zip(proposals, targets, weights):
            if multi_stream:
                s = torch.cuda.Stream()
                s.wait_stream(cs)
                streams.append(s)
            ...
            with torch.cuda.stream(s):
                ...
                if "target_index" in targets_per_image.extra_fields:
                    masks_per_image = _C.global_transforms_generate_mask_targets(
                        targets_per_image.get_field('target_index'),
                        targets_per_image.get_field('after_transforms_img_infos'),
                        targets_per_image.get_field('after_transforms_indexes'),
                        targets_per_image.get_field('after_transforms_dense_xy'),
                        clamped_idxs,
                        weights_per_image.float(),
                        proposals_per_image.bbox, # anchors
                        targets_per_image.get_field('max_num_polygons'),
	...
	# negative sampls are already filtered out, so all samples are positive
        ...
        labels_pos, mask_targets = self.prepare_targets(proposals, targets, self.syncfree, weights)
