...
from evaluation.TFinception_V3 import InceptionV3, SwAV, TFInceptionV3, Vgg16
...
def compute_fid_data(
    ...
    **kwargs,
    ...
    Returns:
        (float): FID value.
    """
    print_flush("Computing FID.")

    # Loading saved COCO stats
    if True:
        coco_stats = os.path.join(folder_to_store_real_act, "val2014_512x512_30k_stats.npz")

        # Load the statistics
        assert os.path.exists(coco_stats), f"Could not find {coco_stats}"
        print_flush(f"Loading statistics from {coco_stats}")

        with np.load(coco_stats) as f:
            m, s = f["mu"][:], f["sigma"][:]

        print_flush(f"Computing statistics for the model")
        ...
        print_flush(f"Printing shapes")
        print_flush(act_b.shape)
        act = act_b.detach().cpu().numpy()

        print_flush(act_b.shape)
        computed_m = np.mean(act, axis=0)
        computed_d = np.cov(act, rowvar=False)

        FID = calculate_frechet_distance(m, s, computed_m, computed_d)
        return FID
    # else:
    ...
    #         **kwargs,
    #     )
    #     print_flush(act_a.shape, act_b.shape)
    #     if is_master():
