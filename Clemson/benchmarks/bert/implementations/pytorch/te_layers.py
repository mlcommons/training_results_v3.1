...
from transformer_engine.pytorch.cpp_extensions import fused_attn_fwd, fused_attn_bwd
...
class _MHA(torch.autograd.Function):
    ...
    def forward(
        ...
        context_, M, Z, philox_unpacked = fused_attn_fwd(
            qkv_out,
            fp8_dtype_forward,
            cu_seqlens,
    ...
    def backward(
        ...
        with _prepare_backward(True, ctx.fp8_meta, name="_MHA"):
            (
                inputmat_t,
                qkv_weight_t_fp8,
                workspace,
            ...
            dqkv = fused_attn_bwd(qkv_out,
                ...
                ctx.fp8_meta['scaling_bwd'].scale[META_DP], # q_scale_ds
                ctx.fp8_meta['scaling_bwd'].scale[META_DQKV], # q_scale_dqkv
                ctx.fp8_meta['scaling_bwd'].amax_history[0][META_DP], # amax_ds
...
class _LayerNormMLP(torch.autograd.Function):
    ...
    def backward(
        ...
        with _prepare_backward(True, ctx.fp8_meta, name="_LayerNormMLP"):
            (
                inputmat,
                ln_weight,
                mu,
