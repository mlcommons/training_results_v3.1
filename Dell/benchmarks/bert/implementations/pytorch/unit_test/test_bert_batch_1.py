...
class TestBert(unittest.TestCase):
  ...
  def setUpClass(self):
    ...
    self.bert_config.fused_bias_mha = False
    self.bert_config.fused_bias_fc_loss_head = False
    self.bert_config.fused_gemm_gelu = False
    self.bert_config.max_seq_length = 512
    self.bert_config.apex_softmax = False
    self.bert_config.enable_stream = True
    self.bert_config.packed_samples = False
    # FP8 recipe
    self.bert_config.use_transformer_engine2 = False
  ...
  def test_bert_model_sequence_output(self):
    ...
    test_pass = bins_absolute[-1] < 2.e-4  # WHAT HAPPEANED HERE ? A100 exception
  ...
  def test_bert_mlm_prediction_scores(self):
    ...
    tf_result = tf_result[masked_lm_positions.view(-1).nonzero(as_tuple=True)[0].cpu(),:]
