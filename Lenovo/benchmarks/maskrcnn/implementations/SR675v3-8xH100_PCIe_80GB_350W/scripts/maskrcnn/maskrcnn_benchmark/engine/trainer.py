...
def check_sustained_training_time(sustained_training_time, raw_train_start):
    if is_main_process():
        print(f"Training runs {(time.time() - raw_train_start) / 60} mins")
...
def do_train(
    ...
    sustained_training_time = arguments["sustained_training_time"]
    need_log_run_stop = True
    model.enable_train()
    ...
    for iteration, (images, targets) in enumerate(prefetcher, start_iter):
        ...
        if per_iter_end_callback_fn is not None:
            ...
            if early_exit and need_log_run_stop:
                mllogger.log_run_stop(status=mllogger.constants.SUCCESS, throughput=throughput)
                need_log_run_stop = False
            if check_sustained_training_time(sustained_training_time, start_training_time) and early_exit:
                break

    if final_callback_fn is not None and not early_exit:
        if final_callback_fn():
            early_exit = True
            if need_log_run_stop:
                mllogger.log_run_stop(status=mllogger.constants.SUCCESS, throughput=throughput)
                need_log_run_stop = False
