...
class GraphableTrainBS1GT(nn.Module):
    def __init__(self, cfg):
        super(GraphableTrainBS1GT, self).__init__()
        ...
        self.nhwc = cfg.NHWC
        _, _, _, _, self.max_num_polygons = cfg.DATALOADER.MASK_LOSS_FROM_GLOBAL_NUMELS
        self.stream1 = torch.cuda.Stream()
    ...
    def forward(self, images_tensor, image_sizes_tensor, target_bboxes, target_objectness, target_labels, target_index, transformed_img_infos, indexes, transformed_dense_coordinates):
        current_stream = torch.cuda.current_stream()
        features = self.backbone(images_tensor)
        features_nchw = [feature.permute([0,3,1,2]) for feature in features] if self.nhwc else features
        self.stream1.wait_stream(current_stream)
        ...
        current_stream.wait_stream(self.stream1)
        batched_anchor_data = [anchor_boxes, anchor_visibility, [(1344,800) for _ in range(image_sizes_tensor.shape[0])]]
        #print("target_bboxes[0] = %s" % (str(target_bboxes[0])))
        target = BoxList(target_bboxes[0], batched_anchor_data[2][0], "xyxy")
        target.add_field('labels', target_labels)
        target.add_field('target_index', target_index)
        target.add_field('after_transforms_img_infos', transformed_img_infos)
        target.add_field('after_transforms_indexes', indexes)
        target.add_field('after_transforms_dense_xy', transformed_dense_coordinates)
        target.add_field('max_num_polygons', self.max_num_polygons)
        targets = [target_bboxes, target_objectness, target_labels, [target]]
        self.stream1.wait_stream(current_stream)
        with torch.no_grad():
            proposals = self.rpn.box_selector_train(
                batched_anchor_data, objectness, rpn_box_regression, image_sizes_tensor, targets
        ...
        return loss_objectness, loss_rpn_box_reg, loss_classifier, loss_box_reg, loss_mask['loss_mask']
...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(self, cfg, spatial_blob, batch_size_one):
        ...
        self.mask_loss_from_global = cfg.DATALOADER.MASK_LOSS_FROM_GLOBAL_TENSORS
        self.precompute_rpn_constant_tensors = cfg.PRECOMPUTE_RPN_CONSTANT_TENSORS
        if self.mask_loss_from_global:
            _, self.after_transforms_img_infos_numel, self.indexes_numel, self.after_transforms_dense_xy_numel, self.max_num_polygons = cfg.DATALOADER.MASK_LOSS_FROM_GLOBAL_NUMELS
        if self.batch_size_one:
            if self.mask_loss_from_global:
                self.graphable = GraphableTrainBS1GT(cfg)
                _, self.after_transforms_img_infos_numel, self.indexes_numel, self.after_transforms_dense_xy_numel, self.max_num_polygons = cfg.DATALOADER.MASK_LOSS_FROM_GLOBAL_NUMELS
            else:
    ...
    def forward(self, images, targets=None):
        ...
        if self.training and self.batch_size_one and self.mask_loss_from_global:
            flat_res = self.graphable(images.tensors, images.image_sizes_tensor, targets[0], targets[1], targets[2],
                    targets[3][0].get_field("target_index"),
                    targets[3][0].get_field("after_transforms_img_infos"),
                    targets[3][0].get_field("after_transforms_indexes"),
                    targets[3][0].get_field("after_transforms_dense_xy"))
            loss_classifier, loss_box_reg, loss_objectness, loss_rpn_box_reg, loss_mask = flat_res
            loss_box = dict(loss_classifier=loss_classifier, loss_box_reg=loss_box_reg)
            losses = {}
            losses.update(loss_box)
            ...
            if self.training and self.batch_size_one:
                flat_res = self.graphable(images.tensors, images.image_sizes_tensor, targets[0], targets[1], targets[2])
