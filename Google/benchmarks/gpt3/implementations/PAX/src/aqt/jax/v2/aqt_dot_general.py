...
def _scale_trans(x, ca, ba):
  """Transpose and reshape scale."""
  ca = list(ca)
  ba = list(ba)
  baca = set(ca + ba)
  for i in ca:
    assert x.shape[i] == 1
  ra = [i for i in range(len(x.shape)) if i not in baca]
...
def _make_dot_general_raw(gcfg: config.DotGeneralRaw):
  ...
  def my_dot_general(
    ...
    print(
        'QQ vanilla-beg'
        f' lhs.shape={lhs.shape} rhs.shape={rhs.shape} dn={dimension_numbers}'
    )
    ...
    print(f'QQ vanilla-end out.shape={out.shape}')

    return out, res

  if gcfg.contraction_axis_local_aqt_factor is None:
    return my_dot_general

  # chcemy upewnic sie ze fwd_quant is off.
  # checmy upewnic sie ze fwd nie factorujemy
  ...
  def factored_dg(
    ...
    factor = gcfg.contraction_axis_local_aqt_factor
    assert factor is not None

    if isinstance(rhs, MultiTensor):
      # We are in gradient code.
      fwd_quantized = rhs.qx is not None
      expect_fwd_quantized = gcfg.rhs.use_fwd_quant is not None
      msg = (
          'Misconfiguration: use_fwd_quant=True, but there is no fwd'
          ' quantization (but rhs.qx is None).'
      )
      assert fwd_quantized == expect_fwd_quantized, msg
      if gcfg.rhs.use_fwd_quant:
        assert rhs.qx is not None, msg
        lhs = _maybe_mul(lhs, rhs.qx.qvalue_scale_t)
        rhs = rhs.qx.qvalue
      else:
      ...
      assert gcfg.rhs.use_fwd_quant is None, 'cannot set use_fwd_quant in fwd'
    ...
    def factor_reshape(x, ca, ba):
      ...
      # for consistency of the original cl/568939926
      if len(ca) == 0:
        print(
            'QQ WARNING: there are no contraction axes. This is highly unsual.'
        )
        return x, ca, ba
      shape = list(x.shape)
      ax = ca[0]
      print(
          f'QQ factor={factor}, shape={shape}, ca={ca}, ax={ax}'
          f' ba={ba} orig_size=shape[ca[0]]'
      )
      orig_size = shape[ax]
      ...
      print(f'QQ CA {ca},{ba} -> {new_ca}, {new_ba} ; {x.shape} -> {shape}')
      return x.reshape(shape), new_ca, new_ba

    lhs_f, new_lhs_ca, new_lhs_ba = factor_reshape(lhs, lhs_ca, lhs_ba)
    rhs_f, new_rhs_ca, new_rhs_ba = factor_reshape(rhs, rhs_ca, rhs_ba)

    new_dimension_numbers = (new_lhs_ca, new_rhs_ca), (new_lhs_ba, new_rhs_ba)
    # We are ignoring res, because we are not supporting local AQT in fwd pass.
    out, _ = my_dot_general(lhs_f, rhs_f, new_dimension_numbers, context)
    print(f'QQ factor-end out.shape={out.shape}')
...
def _dot_general_raw_attach_gradient(
  ...
  def make_fwd(return_residual):
    def fwd(
      ...
      print(f'QQ fwd-end ret.shape={ret.shape}')
  ...
  def vjp_bwd(
    ...
    def grad_dot_general(
      ...
      print(
          f'QQ grad-beg G * y_is_lhs={y_is_lhs}; g.shape={g.shape};'
          f' y.shape={y_res.mt.x.shape} dims={dims}'
      )
      out, _ = dot_general(g, y_res.mt, dims, context)
      print(f'QQ grad-end out.shape={out.shape}')
      ...
      print(f'QQ grad-post transposed_out.shape={transposed_out.shape}')
