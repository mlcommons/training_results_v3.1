...
_cont_mounts=("--volume=${DATADIR}:/data" "--volume=${LOGDIR}:/results" "--volume=${PKLDIR}:/pkl_coco" "--volume=${COCO2017PYT_DIR}:/coco_train2017_pyt")
...
mllogger.event(key=mllogger.constants.CACHE_CLEAR, value=True, stack_offset=0)"
      ...
      docker exec  "${_config_env[@]}" "${_cont_name}"  \
           ...
           "/results/${DATESTAMP}_${_experiment_index}.txt" \
      || true
      fi
done
