...
def _add_logging_args(parser):
    ...
    group.add_argument('--mllog-output-path', type=str, default="/tmp/result_0.txt",
                       help='Path to mllog output file. Defaults to /tmp/result_0.txt.')
    return parser
...
def _add_training_args(parser):
    ...
    group.add_argument("--device-warmup", action='store_true',
                       help="Enable device warmup, which executes given number of iterations of training and evaluation in order to create graphs before timer starts")
    group.add_argument("--device-warmup-iterations", type=int, default=5,
                       help="Number of warmup iterations before training.")
...
def _add_data_args(parser):
    ...
    group.add_argument('--test-data-path', nargs='*', default=None,
                       help='Path to the test dataset. Accepted format:'
                            ...
                            'form: dataset1-weight dataset1-path dataset2-weight '
                            'dataset2-path ...')
    group.add_argument('--warmup-dataset-path', type=str, default="/tmp/synthetic_text_document",
                       help='Path to a preexisting synthetic warmup dataset. \
                       Needs to point to a file name without an extension. Defaults to /tmp/synthetic_text_document.')
