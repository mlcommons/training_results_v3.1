...
class FusedCoreAttention(MegatronModule):
    ...
    def sub_forward(self, query_layer, key_layer, value_layer, attention_mask,
        ...
        # [sk, b, np, hn] -> [sk, b * np, hn] -> [b * np, sk, hn]
        query_layer_transposed = query_layer.permute(1,0,2)
        key_layer_transposed = key_layer.permute(1,0,2)
        ...
        # [sk, b, np, hn] -> [sk, b * np, hn] -> [b * np, sk, hn]
        value_layer_transposed = value_layer.permute(1,0,2)

        context_layer = FusedSDPA.apply(query_layer_transposed, \
                                        key_layer_transposed, \
                                        value_layer_transposed, \
                                        None, self.attention_dropout, \
                                        True)
...
class ParallelAttention(MegatronModule):
    ...
    def __init__(self, init_method,
        ...
        if args.use_fused_sdpa:
            assert self.checkpoint_core_attention is True, \
                "FusedSDPA is only compatible with activation checkpointing"
            assert args.position_embedding_type is PositionEmbeddingType.learnable, \
                  "FusedSDPA is only compatible with learnable embeddings"
