...
class AllGatherHandle:
    ...
    def __init__(self, handle, param: Parameter, use_hpu=False, quantization=None) -> None:
        if param.ds_status != ZeroParamStatus.INFLIGHT:
            raise RuntimeError(f"expected param {param.ds_summary()} to be available")
    ...
    def wait(self) -> None:
        if not self.use_hpu:
            ...
            if self.quantization:
                ...
                self.param.data = self.quantization.backend.dequantize(
                    self.quantization.quantized_param, self.quantization.scale_buffer).to(self.__param.device)
        ...
        def all_gather_coalesced(params: Iterable[Parameter],
            ...
            if len(params) == 1:
                ...
                if not quant:
                    ...
                    return AllGatherHandle(handles, param, use_hpu=self.use_hpu)
                else:
                    ...
                    return AllGatherHandle(handle, param, quantization=quant_info, use_hpu=self.use_hpu)
