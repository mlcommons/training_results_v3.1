...
class BF16_Optimizer(ZeROOptimizer):
    ...
    def get_grads_for_norm(self, for_clipping=False):
        ...
        # Performing all reduce on the TP axis for sequence_parallel gradients
        # TODO : coalesce the all reduce operation by flattening the gradients before all reduce and unflattening after

        if for_clipping == False:
            for i, group in enumerate(self.bf16_groups):
                for j, lp in enumerate(group):
                    if getattr(self.bf16_groups[i][j], 'sequence_parallel', False):
                        dist.all_reduce(self.fp32_groups_gradients[i][j],
