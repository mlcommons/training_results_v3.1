...
def run_bias_gelu(inference_module, activations, bias):
...
def run_bias_gelu_ds(activations, bias):
    ...
    return run_bias_gelu(inference_module, activations, bias)


def run_bias_gelu_torch(activations, bias):
    global inference_torch_module
    if inference_torch_module is None:
        inference_torch_module = TorchInferenceOpBuilder().load()
    return run_bias_gelu(inference_torch_module, activations, bias)
...
def test_bias_gelu(batch, sequence, channels, dtype):
    ...
    torch_out = run_bias_gelu_torch(activations_torch, bias_torch)
