...
def run_bias_add(inference_module, activations, bias):
...
def run_bias_add_ds(activations, bias):
    ...
    return run_bias_add(inference_module, activations, bias)


def run_bias_add_torch(activations, bias):
    global inference_torch_module
    if inference_torch_module is None:
        inference_torch_module = TorchInferenceOpBuilder().load()
    return run_bias_add(inference_torch_module, activations, bias)
...
def test_bias_add(batch, sequence, channels, dtype):
    ...
    torch_out = run_bias_add_torch(activations_torch, bias_torch)
    ref_out = run_bias_add_reference(activations_ref, bias_ref)
    if not allclose(ds_out, ref_out):
        print((ds_out - ref_out).abs().max())
        assert (allclose(ds_out, ref_out))
    if not allclose(torch_out, ref_out):
        print((torch_out - ref_out).abs().max())
