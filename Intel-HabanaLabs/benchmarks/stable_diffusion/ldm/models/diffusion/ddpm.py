...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        if validation_config is not None:
            if validation_config["sampler"] == "plms":
                ...
                device = torch.device("hpu" if self.hpu else "cuda")
                self.sampler = DDIMSampler(self, device=device)
            else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.log_freq > 0 and batch_idx % self.log_freq == 0:
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        super().__init__(conditioning_key=conditioning_key, use_fp16=use_fp16, use_autocast=use_autocast, *args, **kwargs)
        ...
        if self.ckpt is not None:
            ...
            if self.reset_ema:
                ...
                rank_zero_info(
                    f"Resetting ema to pure model weights. This is useful when restoring from an ema-only checkpoint.")
                self.model_ema = LitEma(self.model)
        if self.use_autocast and self.hpu_graph:
            self.cond_stage_config["params"].update({"hpu_graph":True})
            import habana_frameworks.torch.hpu.graphs as htgraphs
            htgraphs.ModuleCacher()(model=self.model, have_grad_accumulation=False)
    ...
    def forward(self, x, c, *args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device, dtype=torch.int32)
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False, sqrt_alphas_cumprod=None, sqrt_one_minus_alphas_cumprod=None):
        ...
        cond['sqrt_alphas_cumprod'] = sqrt_alphas_cumprod
        cond['sqrt_one_minus_alphas_cumprod'] = sqrt_one_minus_alphas_cumprod
        x_recon, noise = self.model(x_noisy, t, **cond)

        if isinstance(x_recon, tuple) and not return_ids:
            return x_recon[0], noise
        else:
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        model_output, noise = self.apply_model(x_start, t, cond, sqrt_alphas_cumprod=self.sqrt_alphas_cumprod,
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None, sqrt_alphas_cumprod=None, sqrt_one_minus_alphas_cumprod=None):
        if self.conditioning_key is None:
            ...
            if not self.sequential_cross_attn:
                ...
                cc = c_crossattn
            out, noise = self.diffusion_model(x, t, context=cc, sqrt_alphas_cumprod=sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod=sqrt_one_minus_alphas_cumprod)
