...
machine with:
1. [PyTorch 23.09-py3 NGC container](https://ngc.nvidia.com/registry/nvidia-pytorch)
...
On Fujitsu's benchmark execution script described later assumues the pretrainied backbone 
will be stored in `./torch-model-cache/hub/checkpoints`.
...
The benchmark is run with the benchmark execution scripts, named `do_ssd.sh`,

The script sets execution parameters, the number of benchmark runs, the place of dataset,
the place of benchmark logs and etc.,
...
These are the lowest level classes (no child classes)  the dataset
