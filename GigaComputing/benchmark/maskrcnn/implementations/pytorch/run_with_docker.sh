...
_cont_mounts=("--volume=${DATADIR}:/data" "--volume=${LOGDIR}:/results" "--volume=${PKLPATH}:/pkl_coco" "--volume=$(pwd):/workspace/object_detection" "--volume=${coco_train}:/coco_train2017_pyt")
...
docker run --rm --init --detach --gpus all \
    ...
    --ulimit=stack=67108864 --ulimit=memlock=-1 -e MLPERF_SUBMISSION_ORG=GigaComputing -e MLPERF_SUBMISSION_PLATFORM=G593-ZD2 \
