...
def parse_args():
    ...
    training.add_argument('--extend_run_to_minutes', default=0, type=float,
                          help='Extends training so that it runs at least the given number of minutes')
...
def main():
    ...
    can_finish_training_nontime_based = False
    can_finish_training_overall = False

    for epoch in count(start=start_epoch + 1):
        ...
        # we need to broadcast the time since train start from rank 0,
        # but we don't want to do unnecessary syncs before RUN_STOP,
        # so we do broadcasting only if we are sure training can be actually finished
        if not can_finish_training_nontime_based:
            can_finish_training_nontime_based = True  # will stay True if one of the conditions is met
            ...
            elif args.steps_this_job is not None and step > args.steps_this_job:
                print_once(f'Finished after {step - 1} steps (steps_this_job reached).')
            elif 0 < args.epochs_this_job <= epoch - start_epoch:
                print_once(f'Finished after {args.epochs_this_job} epochs (epochs_this_job reached).')
            elif epoch >= args.epochs:
                print_once(f'Finished after {epoch} epochs (max epochs: {args.epochs}).')
            else:
                can_finish_training_nontime_based = False

            if can_finish_training_nontime_based:
                if args.extend_run_to_minutes:
                    print_once(f'Continuing training until {args.extend_run_to_minutes} minutes.')
                else:
                    can_finish_training_overall = True

        if can_finish_training_nontime_based and not can_finish_training_overall:
            # Main part of training is done, we can afford broadcasting here
            train_time_minutes = (time.time() - training_start_time) / 60
            if multi_gpu:
                time_tensor = torch.tensor([train_time_minutes], dtype=torch.float, device='cuda')
                torch.distributed.broadcast(time_tensor, 0)
                train_time_minutes = time_tensor[0].item()

            if train_time_minutes > args.extend_run_to_minutes:
                print_once(f'Stopping training after {train_time_minutes:.2f} minutes (extension time {args.extend_run_to_minutes})')
                can_finish_training_overall = True
            elif epoch >= args.epochs:
                print_once(f'Warning: extension beyond max epochs not implemented. Stopping training early.')
                can_finish_training_overall = True

        sbridge.stop_epoch_prof()
        if can_finish_training_overall:
            break
        # end of epoch

    assert can_finish_training_nontime_based and can_finish_training_overall, \
        f'All finish flags must be True: {can_finish_training_nontime_based} and {can_finish_training_overall}'

    training_time = time.time() - training_start_time
    log((), None, 'train_avg', {'throughput': training_utts / training_time})

    if not is_target_reached and last_wer > args.target:
