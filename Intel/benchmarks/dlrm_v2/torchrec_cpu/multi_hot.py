...
class Multihot:
    ...
    def __make_offsets(
        ...
        lS_o = torch.cumsum(torch.concat((torch.zeros(len(multi_hot_sizes)).view(-1,1), lS_o.view(-1, batch_size)), dim=1), axis=1)
        return lS_o
    ...
    def convert_to_multi_hot(self, batch):
        batch_size = len(batch[0])
        lS_i, lS_o = self.__make_new_batch(batch[1], batch_size)
        return (batch[0], list(lS_i), list(lS_o), batch[2])
